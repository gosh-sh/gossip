...
async fn spawn_nodes(num_nodes: u16, transports: Vec<&dyn Transport>) -> Vec<ChitchatHandle> {
    let mut handles = Vec::new();
    for chitchat_id in 0..num_nodes as usize {
        let handle = spawn_one(chitchat_id as u16, transports[chitchat_id]).await;
        handles.push(handle);
    }
    handles
}
...
async fn delay_before_detection_sample(
    ...
    assert!(num_nodes > 2);
    let mut handles = spawn_nodes(num_nodes as u16, transports).await;
...
async fn spawn_quic_transport(ip_addr: String, n: usize) -> Vec<QuicTransport> {
    ...
    for i in 0..n {
        ...
        let transport = gossip_transport::channel::ChannelTransport::new(
            incoming_messages,
            outgoing_messages,
            tokio::sync::watch::channel(Instant::now()).0,
        );

        transports.push(QuicTransport { advertise_addr, bind_addr, quic_handler, transport });
    }
    transports
}
...
async fn test_delay_before_dead_detection_10() {
    ...
    let transports = spawn_quic_transport("127.0.0.1".to_string(), 10).await;
    ...
    for transport in transports {
        ...
        transport.quic_handler.await.unwrap().unwrap();
    }
}
